...
- STAFF logs in with a nickname, not an email - Supabase Auth is still email-based under the hood, so `src/lib/staffLogin.ts` derives a deterministic synthetic email (`nickname@staff.konobar.internal`) at both account-creation and login time. No DB lookup needed to resolve one from the other, and no RLS change was needed for login itself since it's plain `signInWithPassword`. OWNER/SUPER_ADMIN are unaffected and still use real email.
...
**Sprint 7.1 — Staff account creation**: SUPER_ADMIN and OWNER can both create STAFF accounts now (previously only SUPER_ADMIN could create OWNER accounts). Extracted the create-Auth-user-then-insert-profile-with-rollback logic shared by both flows into `src/lib/staffAccount.ts`. New `profiles_owner_insert_staff` RLS policy (`0011_owner_staff_management.sql`) lets OWNER insert only `role = 'STAFF'` rows scoped to their own `restaurant_id` — still can't create another OWNER, a SUPER_ADMIN, or touch another restaurant. New `/owner/staff` page (list + create), and a "Create Staff" button alongside "Create Owner" on `/super-admin`.

**Sprint 7.2 — Staff login by nickname**: STAFF accounts now log in with a unique nickname + password instead of an email (OWNER/SUPER_ADMIN unchanged). Added `profiles.nickname` (unique, format-checked) via `0012_staff_nickname_login.sql`. `/login` now accepts "Email or Nickname" in one field. See the `src/lib/staffLogin.ts` note in Architecture notes for how the email-based Supabase Auth requirement is bridged.
